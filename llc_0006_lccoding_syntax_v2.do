...
/* Written by Anika Knuppel and Dylan Williams (dylan.williams@ucl.ac.uk) at the MRC Unit for Lifelong Health and Ageing at UCL, 2022 - 2023
*/

**# Bookmark #1
*Coding based on ICD-10
clear
cd "S:\LLC_0006\data\views_20231104\"
	use "LLC_0006.nhsd_HESAPC_v0002.dta"
	merge m:m epikey using "LLC_0006.nhsd_HESAPC_otr_v0002.dta"
	...
	* UPDATE Decemeber 2023: adding check for ICD10 code suggested by lancet reviewer -- B94.8 -- which was used in the US to code some LC cases
		gen apc_inf_para_defined_=0
	label var apc_inf_para_defined_ "other_inf_para"
	gen apc_inf_para_defined_date =0
	label var apc_inf_para_defined_date "date of other_inf_para"
	format apc_inf_para_defined_date %td
	
		foreach a in 01 02 03 04 05 06 07 08 09 10 11 12 {
						   replace apc_inf_para_defined_=1 if substr(diag_4_`a',1,4)=="B948"
	* sequelae of other specified infectious and parasitic diseases
						   replace apc_inf_para_defined_date=admidate if substr(diag_4_`a',1,4)=="B948"
	}
	...
	* Marking out earliest occurrence of codes:
	...
	by llc: egen apc_firstdate_inf_para_defined_= min(apc_inf_para_defined_date) if apc_inf_para_defined_==1
	format apc_firstdate_inf_para_defined_ %td
	
	*Combining all info:
	foreach domain in DomainCovid DomainLCovid LC_defined inf_para_defined {
	...
	*drop *_date
	...
	save "S:\LLC_0006\data\derived\ForPaper\views_20231104\HESAPC_data.dta", replace
	****************************************************
**# 2. Preparation of GDPPR#
	*Coding based on GPDPR_Snomed
	cd "S:\LLC_0006\data\views_20231104\"
	clear
	use "LLC_0006.nhsd_GDPPR_v0002.dta"
	...
	* Creating wide dataset including only a marker of domains and earliest date of these:
	...
	save "S:\LLC_0006\data\derived\ForPaper\views_20231104\GDPPR_data.dta", replace
 ...
 *Inclusion in NHS EHR data
	...
	use "S:\LLC_0006\data\views_20231104\LLC_0006.CORE_nhsd_derived_indicator_v0005_20221217.dta"
	
	merge 1:1 llc using "S:\LLC_0006\data\views_20231104\LLC_0006.CORE_denominator_file1_20220122.dta", keepusing(nhs_e)
	
	keep if _m==3
	
	tab nhs_e // all individuals flagged with NHS England linkage permission
	
	
	*Generate dob (set at day 1) for age 

	gen doby=real(substr(dob_year_month,1,4))
	gen dobm=real(substr(dob_year_month, 5,2))
	...
	replace ethnic2_nhs = . if ethnic2==9 |  ethnic2==10
	...
	* Deaths:
	merge 1:1 llc_0006_stud_id using   "S:\LLC_0006\data\views_20231104\LLC_0006.nhsd_MORTALITY_v0002_20230302.dta", gen(mergedmortality)
	drop if mergedmortality==2
	gen deceased=0
	replace deceased=1 if mergedmortality==3
	
	
	keep llc_0006_stud_id dob deceased ethnic2_nhs sex_nhs 
	
	rename llc_0006_stud_id LLC_0006_stud_id
	
	save "S:\LLC_0006\data\derived\ForPaper\views_20231104\EHR_inclusion.dta", replace
	****************************************************
**# 4. Preparation of Cohort dataset#
	*Cohortdata preparation
	*Project:
	local project "_0006"
	cd "S:\LLC`project'\data\views_20231104\"
	*************************************************************
			*BCS70
	*************************************************************
	*LC wave: Wave3
	...
	*LongCovid
	...
	gen study=1
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\BCS70.dta", replace
	clear
	*************************************************************
			*MCS
	*************************************************************
	...
	*LC
	...
	keep LLC`project'_stud_id longcovid3 longcovid3date
	gen study=2
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\MCS.dta", replace
	clear
	*************************************************************
			*NCDS
	*************************************************************
	*LC wave: Wave3
	...
	gen study=3
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\NCDS.dta", replace
	clear
	*************************************************************
			*NEXTSTEP
	*************************************************************
	*LC wave: Wave3
	...
	keep LLC`project'_stud_id longcovid3date longcovid3 
	gen study=4
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\NEXTSTEP.dta", replace
	clear
	...
	*LC wave: Wave3
	...
	keep LLC`project'_stud_id longcovid3date longcovid3 
	gen study=5
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\NSHD.dta", replace
	clear
	...
	*Several waves with long Covid questions
	use LLC_0006.UKHLS_cg_indresp_w_v0006_20230614.dta, replace
	local a "cg"
	...
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\UKHLS_`a'.dta", replace
	clear
	use LLC_0006.UKHLS_ch_indresp_w_v0006_20230614.dta
	local a "ch"
	recode `a'_longcovid (1=0 "recovered")(2=1 "not recovered") (-9/-1=.), gen(`a'_covidrecov) label(covidrecovL)
	tab `a'_covidrecov
	recode `a'_cvtime (-9/-1=.) (1/3=0 "nod") (4/11=1 "4-<12wks") (12/100=2 "12+ weeks"), gen(`a'_longcovid3) label(longcovid3l)
	...
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\UKHLS_`a'.dta", replace
	clear
	use LLC_0006.UKHLS_ci_indresp_w_v0006_20230614.dta
	local a "ci"
	recode `a'_longcovid (1=0 "recovered")(2=1 "not recovered") (-9/-1=.), gen(`a'_covidrecov) label(covidrecovL)
	tab `a'_covidrecov
	recode `a'_cvtime (-9/-1=.) (1/3=0 "nod") (4/11=1 "4-<12wks") (12/100=2 "12+ weeks"), gen(`a'_longcovid3) label(longcovid3l)
	...
	clear
	*combine waves
	use S:\LLC`project'\data\derived\ForPaper\views_20231104\UKHLS_cg.dta
	foreach a in h i{
	merge m:m LLC`project'_stud_id using "S:\LLC`project'\data\derived\ForPaper\views_20231104\UKHLS_c`a'.dta", gen(merge`a') 
	}
	gen longcovid3=0
	gen age=.
	...
	*Highest length of symptoms reported at first time
	...
	gen study=6
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\UKHLS.dta", replace
	clear 
	...
	*take into account only time
	...
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\TWINSUK_COPE2.dta", replace
	clear
	*COPE 3
	...
	*bedridden not within the values to added
	...
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\TWINSUK_COPE3.dta", replace
	clear
	use "S:\LLC`project'\data\derived\ForPaper\views_20231104\TWINSUK_COPE2.dta"
	merge m:m LLC`project'_stud_id using "S:\LLC`project'\data\derived\ForPaper\views_20231104\TWINSUK_COPE3.dta", gen(mergecope3)
	*combine
	...
	gen study=7
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\TWINSUK.dta", replace
	clear
	...
	*Length of symptoms
	...
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\ALSPACG1.dta", replace
	clear
	...
	*Length of symptoms
	...
	save "S:\LLC`project'\data\derived\ForPaper\views_20231104\ALSPACG0.dta", replace
	clear
	****************************************
	* Pulling studies together
		local project "_0006"
	cd "S:\LLC`project'\data\derived\ForPaper\views_20231104"
	use  BCS70.dta
	...
	label val study studyL
		...
		save "S:\LLC`project'\data\derived\ForPaper\views_20231104\cohorts_dataset_England.dta", replace
...
**# 5. Combination and final variable preparation#
	*Data combined
	...
	clear
	cd "S:\LLC_0006\data\derived\ForPaper\views_20231104"
	...
	keep if mergeEHR==3 // only those with linkage to denominator file
	count 
	

	* adding IMD from LSOA dataset
	
	rename LLC llc_0006_stud_id
	destring(llc), replace
	merge 1:1 llc_0006_stud_id using "S:\LLC_0006\data\derived\IMDfromCORE_NHSD_lsoa11_v0001_20221217.dta", keepusing(imd2019_q3)
	...
	*Timing
	...
	gen fuptime=round(((mdy(04, 03, 2023)-longcovid3date)/30.437),.1) // 3rd April last date in GDPPR, as analysed in Dec 2023
	sum fuptime if longcovid2!=.
	sum fuptime if longcovid2!=., det
	tabstat fuptime if longcovid2!=., by(studydet) s(mean sd med min max) // 18 to 32 months
	...
	*Combination of health care based long Covid:
	...
	tab hclongCovid // 98 records in current data
	...
	* Showing coding proportion among most severe LC cases -- debilitating symptoms for 3m
	tab longcovid3f
	tab longcovid3f hclongCovid
	prop hclongCovid , over(longcovid3f) citype(agresti)
	...
	tab sex if longcovid2==1

	prtest hclongCovid if longcovid2==1, by(sex) // NB: need to invert results to provide coding among females relative to males as in results table
	
	recode sex (1 = 2) (2 = 1), gen(inv_sex) 
	
		prtest hclongCovid if longcovid2==1, by(inv_sex) // showing difference in women relative to men
	...
	* by function
	...
	prtest hclongCovid if longcovid2f==1, by(inv_sex) 
	
	prtest hclongCovid if longcovid2f==0, by(inv_sex)
	...
	tab ethnic2 if longcovid2==1
	prtest hclongCovid if longcovid2==1, by(ethnic2) 

	
		prtest hclongCovid if longcovid2==0, by(ethnic2) 
	...
	***** BY AGE *****
	...
	prop hclongCovid , over(longcovid2 agetertiles) citype(agresti) percent
		...
		* testing difference between bottom tertile and middle tertile:
			prtest hclongCovid if longcovid2==1 & agetertiles!=3, by(agetertiles) // need to invert result to give tertile 2 with tertile 1 as reference
	...
	* expressing this % difference with group 1 as reference:
	...
	bysort agetertiles: sum  ageatlcquest
	...
	* Age x debilitating LC:
	...
	logit hclongCovid imd2019_q3 if longcovid2==1
		...
		* middle with bottom as reference
	prtest hclongCovid if longcovid2==1 & imd!=3, by(imd)
	prtest hclongCovid if longcovid2==1 & imd!=3, by(inv_imd) // uses inverted variable to give output as 2 versus 1
	
			* top with bottom as reference
	prtest hclongCovid if longcovid2==1 & imd!=2, by(inv_imd) // uses inverted variable to give output as  3 versus 1
			*middle vs top
	prtest hclongCovid if longcovid2==1 & imd!=1, by(inv_imd) 
